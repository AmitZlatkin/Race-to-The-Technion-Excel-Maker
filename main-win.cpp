...
int main() {
    ...
    printLine("For help, re-run from the terminal and use -h or --help as the first flag.");
    printLine();
    printLine("./RaceXl.exe", ' ');
    std::getline(std::cin, userInput);
    userInput = "./RaceXl.exe " + userInput + " ";  // to ensure the last argument is read correctly
    std::vector<string> argv = splitString(userInput);
    int argc = argv.size();  
    stringsPair splitUserInput = readUserInput(argc, argv);

    string jsonString = splitUserInput.first;
    string outputFilename = splitUserInput.second;
